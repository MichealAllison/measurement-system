  ...
  return (
    <div>
      ...
      <div className="bg-surface border border-line rounded p-6 mt-4 mb-6">
        ...
        <div className="flex gap-2.5 flex-wrap">
          <Link
            ...
            className="bg-indigo hover:bg-indigodeep text-surface2 px-4.5 py-2.5 rounded text-[14.5px] font-medium"
          >
        ...
        {link && (
          <div className="mt-4">
            <div className="bg-surface2 border border-line rounded p-3.5 flex items-center gap-2.5">
              <code className="flex-1 text-[13px] text-indigodeep break-all">{link.url}</code>
              <button
                onClick={copy}
                className="bg-indigo hover:bg-indigodeep text-surface2 px-3 py-1.5 rounded text-[13px]"
              >
                {copied ? 'Copied' : 'Copy'}
              </button>
            </div>
            <p className="text-inksoft text-[12.5px] mt-2">
      ...
      {client.measurements.length === 0 ? (
        <div className="text-center text-inksoft text-[14.5px] border border-dashed border-line rounded p-10">
        ...
        client.measurements.map((m) => (
          <div key={m.id} className="border border-line rounded p-4 mb-3 bg-surface2">
            <div className="flex justify-between items-baseline mb-2.5">
              <div className="font-semibold text-[14.5px]">{formatDate(m.date)}</div>
              <div className="text-[12px] text-brass border border-brass rounded-full px-2.5 py-0.5">
            ...
            {m.notes && <div className="text-[13.5px] text-inksoft italic mt-2.5">{m.notes}</div>}
          </div>
        ))
      )}
    </div>
